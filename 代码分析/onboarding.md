...
CLI 程序启动：cli.ts 第 20 行
Agent 对象创建：sdk.ts 第 297 行
Agent 开始处理问题：agent.ts 第 384 行 prompt()
...
#### session / services / settings 关系

| 概念 | 是什么 | 能干什么 |
| --- | --- | --- |
| **settings** | `settings.json` 里的配置数据 + `SettingsManager` 读写/合并 | 默认模型、transport、retry、扩展路径、主题… **持久化偏好** |
| **services** | 绑 `cwd` 的环境容器，`settingsManager` 是其中一员 | 提供 auth、模型列表、资源发现；**不能对话** |
| **session** | `AgentSession`，上面跑 agent | `prompt()`、工具调用、压缩、分支；持有 `Agent` + `SessionManager` + `settingsManager` |

```mermaid
flowchart TB
  subgraph disk [磁盘]
    SJSON["settings.json 全局+项目"]
    AUTH["auth.json"]
    JSONL["session .jsonl 对话历史"]
  end

  subgraph services [AgentSessionServices 厨房]
  ...
  subgraph session [AgentSession 可对话]
    Agent["Agent 循环"]
    Ext["扩展/工具"]
    SMgr2["settingsManager 引用"]
    SessMgr["SessionManager 引用"]
  end
...
- 在 services 里：services.settingsManager（创建时从磁盘加载）
- 在 session 里：再持有一份引用，运行时读 transport、retry、steering 等

SessionManager 不算 services： 管的是哪条对话（.jsonl 消息树），不同 session 可复用同一套 services。/resume、/fork 换 sessionManager，services 可不变。
